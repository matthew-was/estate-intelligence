# Component 2: Text Extraction, Processing & Embedding — Specification
...
To Internal Stage: Embedding & Storage:
...
**Note:** Low-quality documents proceed through Component 2 but are flagged. Internal Stage: Embedding & Storage will store the quality score for later manual review in a future build phase.
...
- Metadata enables Internal Stage: Embedding & Storage to retrieve full parent context when needed for RAG
...
## Internal Stage: Embedding & Storage

### Critical Inputs Internal Stage: Embedding & Storage Receives

- **Chunks with parent references**: Internal Stage: Embedding & Storage must store the relationship between chunks and original documents
...
- **Treatment tags** (e.g., "metadata chunk", "reference-only"): Internal Stage: Embedding & Storage should respect these in ranking (metadata chunks discoverable but lower weight than content chunks)
- **Domain context feedback list**: Available for Internal Stage: Embedding & Storage to use in entity extraction/disambiguation (Phase 2+)

### Design Questions for Internal Stage: Embedding & Storage
...
4. **Duplicate detection at Stage B**: Embedding similarity for near-duplicates (Phase 3) will require additional logic—should Stage B implement this or should it stay in Component 2?

### Processing Assumptions Internal Stage: Embedding & Storage Can Make
...
- Internal Stage: Embedding & Storage can provide feedback to Component 2 on quality scores (e.g., "embeddings for this chunk are incoherent, quality score might be too high")
- Internal Stage: Embedding & Storage can identify when chunks don't embed well (embedding variance indicators) and flag for review
- Internal Stage: Embedding & Storage could notify Component 2 when domain context updates would help (e.g., "multiple chunks reference unknown entity 'The Mill'")

## Future Integration: Component 1

**Quality Score Review Queue (Future Build Phase)**

- Internal Stage: Embedding & Storage needs ability to surface low-quality documents (below configurable threshold)
...
5. **Output contract satisfied** (Internal Stage: Embedding & Storage receives what it expects)
